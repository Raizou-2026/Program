...
bxCAN::bxCAN(CAN_TypeDef* hcan, GPIOPin_t rx, GPIOPin_t tx)
{
	ch							= hcan;
	can_state					= SLEEPMODE;
	rx_pin						= rx;
...
SysError_t bxCAN::init(uint64_t bitrate)
{
	...
	if(ch == NULL) {
		...
		CANx_TX_IRQn	= CAN_TX_IRQn;
		CANx_RX0_IRQn	= CAN_RX0_IRQn;
		CANx_RX1_IRQn	= CAN_RX1_IRQn;
	}
...
SysError_t bxCAN::init(uint64_t bitrate, GPIOPin_t rx, GPIOPin_t tx)
{
	CANBaudRateVal_t rate;
...
SysError_t bxCAN::singleFilter(CANFilter_t filter)
{
	uint32_t act_filter_num = filter.filter_num;
	...
	ch->FMR |= CAN_FMR_FINIT;
	ch->FA1R &= ~(1UL << act_filter_num);

	if(filter.mode == MASK_MODE) {
		ch->FM1R &= ~(1UL << act_filter_num);
	} else {
		ch->FM1R |= (1UL << act_filter_num);
	}

	if(filter.fifo_assign == FIFO0) {
		ch->FFA1R &= ~(1UL << act_filter_num);
	} else {
		ch->FFA1R |= (1UL << act_filter_num);
	}

	ch->sFilterRegister[act_filter_num].FR1 = 0;
	ch->sFilterRegister[act_filter_num].FR2 = 0;
	if(filter.scale == DUAL16BIT) {
		ch->FS1R &= ~(1UL << act_filter_num);

		ch->sFilterRegister[act_filter_num].FR1 |= ((filter.id & 0x07FF0000UL) << 5);
		ch->sFilterRegister[act_filter_num].FR1 |= ((filter.id & 0x00007FFUL) << 5);
		ch->sFilterRegister[act_filter_num].FR2 |= ((filter.mask & 0x07FF0000UL) << 5);
		ch->sFilterRegister[act_filter_num].FR2 |= ((filter.mask & 0x000007FFUL) << 5);

		if(filter.id_type == STID) {
			ch->sFilterRegister[act_filter_num].FR1 &= ~(1UL << 19);
			ch->sFilterRegister[act_filter_num].FR1 &= ~(1UL << 3);
			ch->sFilterRegister[act_filter_num].FR2 &= ~(1UL << 19);
			ch->sFilterRegister[act_filter_num].FR2 &= ~(1UL << 3);
		} else {
			ch->sFilterRegister[act_filter_num].FR1 |= (((filter.id & (0b111UL << 27)) >> 11) | (1UL << 19));
			ch->sFilterRegister[act_filter_num].FR1 |= (((filter.id & (0b111UL << 11)) >> 11) | (1UL << 3));
			ch->sFilterRegister[act_filter_num].FR2 |= (((filter.mask & (0b111UL << 27)) >> 11) | (1UL << 19));
			ch->sFilterRegister[act_filter_num].FR2 |= (((filter.mask & (0b111UL << 11)) >> 11) | (1UL << 3));
		}

		if(filter.frame_type == DATA_FRAME) {
			ch->sFilterRegister[act_filter_num].FR1 &= ~(1UL << 20);
			ch->sFilterRegister[act_filter_num].FR1 &= ~(1UL << 4);
			ch->sFilterRegister[act_filter_num].FR2 &= ~(1UL << 20);
			ch->sFilterRegister[act_filter_num].FR2 &= ~(1UL << 4);
		} else {
			ch->sFilterRegister[act_filter_num].FR1 |= (1UL << 20);
			ch->sFilterRegister[act_filter_num].FR1 |= (1UL << 4);
			ch->sFilterRegister[act_filter_num].FR2 |= (1UL << 20);
		...
		if(filter.id_type == STID) {
			ch->sFilterRegister[act_filter_num].FR1 |= ((filter.id & 0x7FF) << 21);
			ch->sFilterRegister[act_filter_num].FR2 |= ((filter.mask & 0x7FF) << 21);
		} else {
			ch->sFilterRegister[act_filter_num].FR1 |= ((filter.id & 0x1FFFFFFF) << 3) | (1UL << 2);
			ch->sFilterRegister[act_filter_num].FR2 |= ((filter.mask & 0x1FFFFFFF) << 3) | (1UL << 2);

			if(filter.frame_type == DATA_FRAME) {
				ch->sFilterRegister[act_filter_num].FR1 &= ~(1UL << 1);
				ch->sFilterRegister[act_filter_num].FR2 &= ~(1UL << 1);
			} else {
				ch->sFilterRegister[act_filter_num].FR1 |= (1UL << 1);
				ch->sFilterRegister[act_filter_num].FR2 |= (1UL << 1);
			}
...
SysError_t bxCAN::allFilter(CANFilter_t* pfilter)
{
...
void bxCAN::setCallback(CANCallbackType_t type, CallbackFunc_t func)
{
	switch (type) {
...
SysError_t bxCAN::transmit(CANTxHeader_t *txheader, uint32_t timeout)
{
	uint32_t tsr;
	uint32_t start;
	uint8_t mailbox;
...
SysError_t bxCAN::receive(CANRxHeader_t* rxheader, CANFifo_t fifo)
{
...
SysError_t bxCAN::receiveIT(CANRxHeader_t* rxheader, CANFifo_t fifo)
{
	if(ch == NULL || rxheader == NULL) {
		return SYS_ERROR;
	}
...
void bxCAN::pinInit(void)
{
	pinMode(rx_pin, OTHER);
	pinMode(tx_pin, OTHER);

	pinAFSelect(rx_pin, AF9);
	pinAFSelect(tx_pin, AF9);

	pinSpeed(rx_pin, HIGHSPEED);
	pinSpeed(tx_pin, HIGHSPEED);
}

CANBaudRateVal_t bxCAN::calcRateVal(uint64_t bitrate)
{
